function setup() {
  // write your codes here
  new Canvas(800,400);
  background(250);
  textSize(24);
  fill("red")
  // console.log("Hello World")
//  let a = 1;
//  let b = 2;
//  let sum = (a+b) * (a+b) / b;
//  console.log("A has a value of " + a + " while B has a value of " + b)
//  console.log(sum)
  // --- Exercise: Area of Triangle ---
  // write your codes here
//   let base = 10
//   let height = 5 
//   let area = 0.5 * base * height 
//   console.log("The area of the triangle which has a base of " + base + " and a height of " + height +" has the area of" + area);
// // para for string is string, x, y e.g. ()
// text("Area: " + area, 400,200)
// for (let i = 0; i <= 10; i+= 2) {
//   console.log(i)
// }
  // --- Exercise: Sum of first 10 even numbers ---
  // write your codes here
//   let x = 50
//   let y = 200
//   sum = 0
// for (let i = 0; i <= 20; i+=2) {
//   console.log(i)
//   text(i, x, y)
//   x = x + 50
//   sum = sum +i
// }
//   console.log("Sum of first 10 even numbers " + sum )
//   text("Sum of first 10 even numbers:" + sum, 150, 100)
  // --- Exercise: Age category classification ---
  // write your codes here
// let age = 10
// if (age <= 9) {
//   console.log("Lower Primary")
// } else if (age >= 10 && age <=12) {
//   console.log("Upper Primary")
// }else if (age >= 13 && age <=16 ) {
//     console.log("Secondary School")
//   }else {
//     console.log("N/A.")
//   }


  // --- Exercise: Display odd numbers backward using while loop ---
  // write your codes here
  // let i = 19
  // while(i >=1) {
  //   if (i
  //      % 2 === 1) {
  //     console.log(i)
  //   }else if (num % 2 === 0) {
  //     num -=1
  //   }
  
  // }
  // --- Exercise: Array operations (groceries) ---
  // write your codes here
let groceries = ["apple","bread","milk"];
groceries.push("orange")
groceries.push("butter")
groceries.shift(); // remove first item
// groceries.pop(); // remove last item
// groceries.splice(1,1); // syntax: starting index and how much u want to remove
groceries.splice(1,1, "kaya") // syntax:starting index and how much u want to remove and starting from where and what u wana replace it with - only works for one item
groceries.splice(1,2, "kaya", "lemons", "lime", "bunsen burner", "baking soda", "door", "chair") 
console.log(groceries)
}

